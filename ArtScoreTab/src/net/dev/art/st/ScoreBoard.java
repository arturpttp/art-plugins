...
class ScoreBoard {
	...
	public void definirLinha(String texto, int linha) {
		if (b(c(), linha)) {
			if (a(c(), linha).equalsIgnoreCase(texto))
				return;
			if (!(a(c(), linha).equalsIgnoreCase(texto)))
				c().getScoreboard().resetScores(a(c(), linha));
		}
	...
	public void enviar() {
		...
		if (c().getDisplaySlot() != DisplaySlot.SIDEBAR)
			c().setDisplaySlot(DisplaySlot.SIDEBAR);
	...
	private Objective c() {
		...
		return score.getObjective(recebidor.getName()) == null
				? score.registerNewObjective(recebidor.getName(), "dummy")
				: score.getObjective(recebidor.getName());
	}
}
