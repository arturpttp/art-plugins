...
public class Config {
	...
	private File file;
	private String name;
	protected String prefix = "§aConfig§8 » ";
	...
	public Config reloadConfig() {
		String nome = name.replace(".yml", "") + ".yml";
		String mensagem = "§eCarregando o arquivo `§b" + nome + "§e`";
	...
	public Config saveConfig() {
		...
		try {
			config.save(file);
			Bukkit.getConsoleSender().sendMessage(prefix + "§eSalvando o arquivo `§f" + nome + "§c`");
		} catch (IOException ex) {
			Bukkit.getConsoleSender().sendMessage(prefix + "§cNão foi possivel salvar o arquivo `§f" + nome + "§c`");
		}
		return this;
	}
